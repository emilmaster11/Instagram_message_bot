...
def sendmessages_suna():
    options=webdriver.ChromeOptions()
    options.add_argument("lang=de-DE,de,en-US,en")
    options.add_argument("--user-data-dir=C:\\Users\\emilb\\Desktop\\Python_projekte\\BOTS\\Chrome_Profile\\GoogleChromeProfile3")
    options.add_argument("--profile-directory=Profile3")
    options.add_argument(f'user-agent=Mozilla/5.0 (Linux; Android 10) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/105.0.5195.58 Mobile Safari/537.36')   
    ...
    for user in text:
     ...
     try:  #Nachrichten schreiben Funktion
      ...
      time.sleep(randint(von_zeit1,bis_zeit1))
      nachrichten = ["Hey :) ich hab gesehen, du folgst auch dem Oskar Karem. Ich persönlich finde die Themen Businessgründung und entwicklung super spannend. Sind diese Themen auch für dich relevant?","Hi, \ncooles Profil :) Ich hab gesehen du folgst auch dem Oscar Karem, Ich persönlich finde die Themen Businessgründung und entwicklung super spannend. Sind diese Themen auch für dich relevant? "]
      nachricht_schreiben.send_keys(random.choice(nachrichten))  #Hier kommt die Nachricht rein
...
if __name__ == "__main__":
   ...
   sendmessages_suna()
   time.sleep(300) 
   ###3.Auf einem Server alles laufen lassen
